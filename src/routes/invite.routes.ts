...
const router = Router();
let inviteService: ProfessionalInviteService | null = null;
...
router.post("/test/create-invite", async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: error?.message || "Failed to create test invite code",
    });
  }
});
...
router.post("/test/process-invite", async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: error?.message || "Failed to process test invite code",
    });
  }
});
