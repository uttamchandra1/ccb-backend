...
class CardDetectionService {
    ...
    async scanGmailForCards(userId, oauth2Client) {
        ...
        for (const query of searchQueries) {
            try {
                ...
                console.log(`[GMAIL] Query: ${query} → messages: ${listResp.data.resultSizeEstimate ||
                ...
                for (let i = 0; i < messageIds.length; i += batchSize) {
                    ...
                    const batchPromises = batch.map(async (messageId) => {
                        try {
                            const result = await this.analyzeEmailWithAdvancedFiltering(messageId, gmail);
                            return result;
                        }
                        catch (error) {
                    ...
                    const validResults = batchResults.filter((result) => result !== null && result.matchedCards.length > 0);
    ...
    async analyzeEmailWithAdvancedFiltering(messageId, gmail) {
        var _a;
        ...
        try {
            console.log(`[GMAIL] Analyzing email → subject: "${subject}" | from: "${sender}" | id: ${messageId}`);
        }
        catch (_) { }
        ...
        try {
            console.log(`[GMAIL] Classified → type: ${emailAnalysis.type} bank: ${emailAnalysis.bank} subject: "${subject}"`);
        }
        catch (_) { }
        // Skip promotional and fraudulent emails
        if (emailAnalysis.type === "promotional" ||
            emailAnalysis.type === "fraud") {
            return null;
        }
        // Only process statement and transaction emails
        if (emailAnalysis.type !== "statement" &&
            emailAnalysis.type !== "transaction") {
            return null;
        }
        // Detect cards using advanced pattern matching
        const matchedCards = this.detectCardsWithAdvancedLogic(subject, sender, "", emailAnalysis);
        try {
            console.log(`[GMAIL] Detected cards → count: ${matchedCards.length} subject: "${subject}"`);
        }
        catch (_) { }
    ...
    classifyEmailType(subject, sender, text) {
        var _a;
        ...
        const senderEmail = ((_a = this.extractEmailAddress(sender)) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || sender.toLowerCase();
        // Check if sender is from a legitimate bank
        let detectedBank;
        for (const [bankName, config] of this.bankConfigs) {
            const emailMatches = config.emailPatterns.some((pattern) => pattern.test(sender) || pattern.test(senderEmail));
            const domainMatches = (config.domains || []).some((domain) => senderEmail.endsWith(`@${domain.toLowerCase()}`) ||
        ...
        const transactionScore = config.transactionKeywords.reduce((score, keyword) => {
    ...
    detectBankFromSubject(subject, text) {
    ...
    detectCardsWithAdvancedLogic(subject, sender, text, emailAnalysis) {
        ...
        for (const pattern of config.cardPatterns) {
            ...
            if (match) {
                ...
                if (card) {
                    ...
                    const confidence = emailAnalysis.type === "statement"
        ...
        if (config.name === "Axis Bank") {
            ...
            if (/flipkart\s+axis\s+bank\s+credit\s+card\s+statement/.test(subj)) {
                const cardKey = this.generateCardKey("Axis Bank Flipkart Credit Card", config.name);
                const card = this.cardDatabase.get(cardKey);
                if (card) {
                    matches.push({
                        ...card,
        ...
        if (config.name === "HDFC Bank") {
            ...
            const productMatch = subj.match(/(millennia|regalia|diners|diners\s+club|moneyback|freedom|titanium|platinum|gold|silver|rupay)/i);
            if (productMatch) {
                ...
                const canonical = canonicalMap[token] ||
        ...
        if (config.name === "SBI Card") {
            const subj = subject.toLowerCase();
            if (/sbi\s+card\s+prime/.test(subj) ||
            ...
            const generic = subj.match(/sbi\s+card\s+(simplyclick|elite|advantage|rewards|pulse|unnati|freedom)/i);
            if (generic) {
                const product = generic[1].toUpperCase();
                const canonical = product === "SIMPLYCLICK"
        ...
        const uniqueMatches = matches.filter((match, index, self) => index === self.findIndex((m) => m.name === match.name));
        // Sort by confidence but allow all
        return uniqueMatches.sort((a, b) => (b.confidence || 0) - (a.confidence || 0));
    }
    ...
    extractEmailText(payload) {
        ...
        if (payload.parts) {
            for (const part of payload.parts) {
                ...
                else if (part.mimeType === "text/html" && ((_c = part.body) === null || _c === void 0 ? void 0 : _c.data)) {
    ...
    async addManualCard(userId, cardData) {
        if (!supabase_1.supabaseAdmin)
            return false;
        const { error } = await supabase_1.supabaseAdmin.from("user_cards").upsert({
            user_id: userId,
    ...
    async getUserCardsAll(userId, forceRefresh = false) {
        try {
            if (!supabase_1.supabaseAdmin) {
    ...
    async getUserCardsOptimized(userId, forceRefresh = false) {
        try {
            if (!supabase_1.supabaseAdmin) {
                throw new Error("Supabase admin not configured");
            }
    ...
    async getFriendsCardsOptimized(userId, forceRefresh = false) {
        try {
            if (!supabase_1.supabaseAdmin) {
                throw new Error("Supabase admin not configured");
            }
            ...
            const { data: allUsers, error: usersError } = await supabase_1.supabaseAdmin.auth.admin.listUsers();
            if (usersError) {
            ...
            const friends = allUsers.users
                ...
                return ({
                    id: user.id,
                    name: ((_a = user.user_metadata) === null || _a === void 0 ? void 0 : _a.full_name) ||
                        ((_b = user.email) === null || _b === void 0 ? void 0 : _b.split("@")[0]) ||
                        "Unknown",
                    email: user.email,
                    avatar: (_c = user.user_metadata) === null || _c === void 0 ? void 0 : _c.avatar_url,
        ...
        `)
            ...
            const result = {
                ...
                analytics: {
                    ...
                    averageCardsPerFriend: friendCards.length > 0
                        ? ((allCards === null || allCards === void 0 ? void 0 : allCards.length) || 0) / friendCards.length
                        : 0,
                    mostCommonCardType: this.getMostCommon((allCards === null || allCards === void 0 ? void 0 : allCards.map((c) => c.card_type)) || []),
                    mostCommonBank: this.getMostCommon((allCards === null || allCards === void 0 ? void 0 : allCards.map((c) => c.bank_name)) || []),
                },
            };
            // Cache the result
            this.cardCache.set(cacheKey, {
    ...
    async requestCardUsage(userId, friendId, cardId, requestMessage) {
        try {
            if (!supabase_1.supabaseAdmin) {
                throw new Error("Supabase admin not configured");
            }
            ...
            const { data: existingRequest } = await supabase_1.supabaseAdmin
                .from("card_requests")
                .select("id, status")
            ...
            const { data: request, error: requestError } = await supabase_1.supabaseAdmin
                .from("card_requests")
                .insert({
                requester_id: userId,
    ...
    async getPendingCardRequests(userId) {
        try {
            if (!supabase_1.supabaseAdmin) {
                throw new Error("Supabase admin not configured");
            }
            // Get requests where user is the card owner
            const { data: receivedRequests, error: receivedError } = await supabase_1.supabaseAdmin
                .from("card_requests")
                .select(`
          id,
          requester_id,
        ...
        `)
            ...
            const { data: sentRequests, error: sentError } = await supabase_1.supabaseAdmin
                .from("card_requests")
                .select(`
          id,
          card_owner_id,
        ...
        `)
            ...
            return {
                ...
                totalReceived: (receivedRequests === null || receivedRequests === void 0 ? void 0 : receivedRequests.length) || 0,
                totalSent: (sentRequests === null || sentRequests === void 0 ? void 0 : sentRequests.length) || 0,
            };
        }
        catch (error) {
    ...
    async respondToCardRequest(requestId, userId, action, responseMessage) {
        try {
            if (!supabase_1.supabaseAdmin) {
                throw new Error("Supabase admin not configured");
            }
